...
class CMSSuccessStoryController extends Controller
{
    public function index()
    {
        $resultSet = CMSMediaCenter::where('type', 'success-stories')->get();
        return view('cms.website.media-center.success-story.index', compact('resultSet'));
    }

    public function addMediaCenter()
    {
        return view('cms.website.media-center.success-story.add');
    }

    public function addMediaCenterData()
    {
        request()->validate([
            'title' => ['required', 'max:55', 'unique:cms_media_center,title,NULL,id,type,success-stories'],
    ...
    public function updateMediaCenter($cmsMediaCenterId)
    {
        ...
        return view('cms.website.media-center.success-story.update', compact('updateMediaCenter'));
    }
    ...
    public function updateMediaCenterData($cmsMediaCenterId)
    {
        $updateMediaCenter = CMSMediaCenter::findOrFail($cmsMediaCenterId);
        request()->validate([
            'title' => ['required', 'max:55', 'unique:cms_media_center,title,'.$cmsMediaCenterId.',id,type,success-stories'],
            'description' => ['required','max:1000']
        ]);
