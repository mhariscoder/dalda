...
class CMSInterviewController extends Controller
{
    public function index()
    {
        $resultSet = CMSMediaCenter::where('type', 'interviews')->get();
        return view('cms.website.media-center.interview.index', compact('resultSet'));
    }

    public function addMediaCenter()
    {
        return view('cms.website.media-center.interview.add');
    }

    public function addMediaCenterData()
    {
        request()->validate([
            'title' => ['required', 'max:55', 'unique:cms_media_center,title,NULL,id,type,interviews'],
    ...
    public function updateMediaCenter($cmsMediaCenterId)
    {
        ...
        return view('cms.website.media-center.interview.update', compact('updateMediaCenter'));
    }
    ...
    public function updateMediaCenterData($cmsMediaCenterId)
    {
        $updateMediaCenter = CMSMediaCenter::findOrFail($cmsMediaCenterId);
        request()->validate([
            'title' => ['required', 'max:55', 'unique:cms_media_center,title,'.$cmsMediaCenterId.',id,type,interviews'],
            'description' => ['required','max:1000']
        ]);
