...
class ClaimScholarShipController extends Controller
{
    ...
    public function updateStudentClaim($claimId) {
        $student = User::where('id', Auth::id())->first()->student_id;
        if (empty($student)) {
            return redirect('/student/claim-for-scholarship')->with('error', "You don't have right permission.");
        }
        ...
        return view('frontend.student.scholarship.claim.update', compact('student', 'claim', 'years'));
    }

    public function updateStudentClaimData($claimId) {
        $studentData = array();
        $user = Auth::user();
        if (empty($user->student_id)) {
