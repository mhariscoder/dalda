...
@section('content')
    {{-- @php
        $category = old("category",$cmsImage->imageable_type);
        $university_name = old("university_name",$cmsImage->imageable_id);
        $hospital_name = old("hospital_name",$cmsImage->imageable_id);
        $institute_name = old("institute_name",$cmsImage->imageable_id);
    @endphp --}}
                        ...
                        <div class="form-group col-md-12">
                            ...
                            <select class="form-control category" name="category">
                                ...
                                @foreach($categories as $val)
                                    <option value="{{$val}}" {{ $cmsImage->imageable_type === $val ? "selected" : ""}}>{{$val}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="form-row upload">
                        <div class="form-group col-md-12">
                            <label for="image_file">Upload Image<span
                                        class="required-class"> *</span></label>
                            <input type="file" name="image_file" id="i_file" accept=".jpg,.jpeg,.png" class="form-control"/>
                            @if (!empty($cmsImage->path))
                                <a href="{{$cmsImage->path.'/'.$cmsImage->image_name ?? ''}}" target="_blank" class="btn btn-dark btn-sm mt-2">Image Link</a>
                            @endif
                        </div>
                    </div>
                                            ...
                                            <div class="form-group col-md-12">
                                                ...
                                                <select class="form-control" id="institute_name" name="institute_name">
                                                    ...
                                                    @foreach($institutes as $val)
                                                        <option value="{{$val->id}}" {{ (int)$institute_name === $val->id ? "selected" : ""}}>{{$val->institute_name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        @if(!empty($cmsImage->image_name))
                                            <div class="d-flex mb-2">
                                                <div class="hovereffect">
                                                    <img src="{{Storage::url('uploads/gallery/images/'.$cmsImage->image_name ?? '')}}">
                                                    <div class="overlay">
                                                    </div>
                                                </div>
                                            </div>
...
@push('scripts')
    ...
    <script>
        ...
        function submitFormData() {
            ...
            axios({
                method: 'post',
                url: '/admin/update-gallery-image-list/' + "{{$cmsImage->id}}",
                data: bodyFormData,
