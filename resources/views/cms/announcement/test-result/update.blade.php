...
@section('content')
    ...
    <div class="main-card mb-3 card">
        <div class="card-body">
            ...
            <form action="/admin/update-student-result/{{$testResult->id}}" method="POST" class="needs-validation" novalidate>
                @method('PUT')
                @csrf
                @if ($errors->any())
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-control" name="student_name" required="required">
                            ...
                            @foreach ($students as $val)
                                <option value="{{ $val->id }}"
                                    {{ (int) old('student_name', $testResult->user_id) === $val->id ? 'selected' : '' }}>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select class="form-control" name="exam_name" required="required">
                            ...
                            @foreach ($exams as $val)
                                <option value="{{ $val->id }}"
                                    {{ (int) old('exam_name', $testResult->upload_document_id) === $val->id ? 'selected' : '' }}>
                                    {{ $val->title }}</option>
                            @endforeach
                        </select>
                        <div class="invalid-feedback">
                ...
                <div class="form-row">
                    <div class="form-group col-md-12">
                        ...
                        <input type="number" name="marks" id="marks" class="form-control set_decimal_places"
                            value="{{ old('marks', $testResult->marks) }}" placeholder="Enter Marks" maxlength="10"
                            data-max-digit="4" step="any" required>
                        <div class="invalid-feedback">
                            Please provide valid marks.
                        </div>
                    </div>
                    <div class="form-group col-md-12">
                        <label for="description">Percentage(%) <span class="required-class">*</span></label>
                        <input type="number" name="percentage" id="percentage" class="form-control set_percentage_places"
                            value="{{ old('percentage', $testResult->percentage) }}" placeholder="Enter Percentage"
                ...
                <div class="col-12 mt-3">
                    <div class="card">
                        <div class="card-header  justify-content-between align-items-center">
                            <div class="row">
                                <div class="col-md-6">
                                    <h4 class="card-title">Update Test Result</h4>
                                </div>
                                <div class="col-md-6">
                        ...
                        <div class="card-body">
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-12">
                                        <form action="/admin/update-student-result/{{$testResult->id}}" method="POST">
                                            @method('PUT')
                                            @csrf
                                            @if ($errors->any())
                                                <div class="alert alert-danger">
                                            ...
                                            <div class="form-row">
                                                <div class="form-group col-md-6">
                                                    ...
                                                    <select class="form-control" name="student_name">
                                                        <option value="">Select</option>
                                                        @foreach ($students as $val)
                                                            <option value="{{$val->id}}" {{ (int)old('student_name',$testResult->user_id) === $val->id ? "selected" : ""}}>{{$val->full_name}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="form-group col-md-6">
                                                    ...
                                                    <select class="form-control" name="exam_name">
                                                        <option value="">Select</option>
                                                        @foreach ($exams as $val)
                                                            <option value="{{$val->id}}" {{ (int)old('exam_name',$testResult->upload_document_id) === $val->id ? "selected" : ""}}>{{$val->title}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
                                            ...
                                            <div class="form-row">
                                                <div class="form-group col-md-12">
                                                    <label for="description">Marks <span
                                                                class="required-class">*</span></label>
                                                    <input type="text" name="marks" id="marks" class="form-control set_decimal_places"
                                                           value="{{ old('marks',$testResult->marks) }}" data-max-digit="4" step="any" maxlength="10">
                                                </div>
                                                <div class="form-group col-md-12">
                                                    <label for="description">Percentage(%) <span
                                                                class="required-class">*</span></label>
                                                    <input type="text" name="percentage" id="percentage" class="form-control set_decimal_places"
                                                           value="{{ old('percentage',$testResult->percentage) }}" data-max-digit="4" step="any" maxlength="10">
                                                </div>
                                            </div>
